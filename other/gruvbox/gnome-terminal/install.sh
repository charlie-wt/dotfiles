...
# based on https://github.com/catppuccin/gnome-terminal @ dab9341433eba9cb1ac99df7c80f87c34fb4fd4c
...
if [ -z "$profile" ]; then
    alert -e "Please specify a profile name to make."
    exit 1
fi
...
# (The `echo` concats all the lines in the file together)
dconf write "$profile_path"palette "$(echo $(cat $PWD/palette))"
